...
class ASENEBMethod(NEBMethod):
    ...
    def add_image_force(self, state, tangential_force, tangent, imgforce,
                        spring1, spring2, i, tangent_index):
        ...
        imgforce -= tangent_index*np.vdot(
            spring1.t * spring1.k -
...
class BaseNEB:
    ...
    def get_forces(self, energies, forces, force_index, tangent_index):
        """Evaluate and return the forces."""
        images = self.images
        forces = forces * force_index
...
class DyNEB(BaseNEB):
    ...
    def get_forces(self, energies, forces, force_index, tangent_index):
        forces = super().get_forces(energies, forces, force_index, tangent_index)
        if not self.dynamic_relaxation:
            return forces
